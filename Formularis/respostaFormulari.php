...
<body>
    ...
    for ($n = 1; $n < 4; $n++) {
        if (!empty($_POST["nomPersona{$n}"])) { // Comprovem si hi ha dades d'altres clients
            $edatAfegida = $_POST["anysPersona{$n}"];
            //Comprovem si la persona afegida es estudiant o no
            $esEstudiantAfegida = isset($_POST["esEstudiantPersona{$n}"]) && $_POST["esEstudiantPersona{$n}"] == "estudiant";
            if (isset($_POST["esEstudiantPersona{$n}"]) && $_POST["esEstudiantPersona{$n}"] == "estudiant") {
